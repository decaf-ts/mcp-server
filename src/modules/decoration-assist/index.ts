...
function readFileSafe(filePath: string, encoding: BufferEncoding = "utf8"): string | undefined {
  try { return fs.readFileSync(filePath, { encoding }); } catch { return undefined; }
}

function listFilesRecursive(root: string, matcher?: (p: string) => boolean): string[] {
  const out: string[] = [];
...
const analyzeRepoSchema = z.object({
  repoPath: z.string({ description: "Relative or absolute path to the target repository inside this monorepo, e.g. './decoration'." })
    .min(1, "repoPath is required"),
  includeTests: z.boolean({ description: "If true, analyze the tests directory (if present) to derive expected behaviors." }).default(true),
  includeDocs: z.boolean({ description: "If true, analyze README.md and docs directories to extract documented features." }).default(true),
}).strict().describe("Analyze a local repository (e.g. ./decoration) to extract APIs, features, tests, and documentation cues.");

const enumerateCapabilitiesSchema = z.object({
  repoPath: z.string({ description: "Relative or absolute path to the target repository to enumerate developer-facing capabilities." })
    .min(1, "repoPath is required"),
}).strict().describe("Enumerate the complete set of capabilities a developer is expected to use from the given repository.");

const planFeatureSchema = z.object({
  feature: z.string({ description: "Natural-language description of a developer's requested feature or task to implement using the repository and available MCP tools." })
    .min(5, "feature must describe the goal clearly"),
  repoPath: z.string({ description: "Target repository path providing the library to use, e.g. './decoration'." })
    .default("./decoration"),
}).strict().describe("Plan which MCP tools to use and in what sequence to implement a requested feature using the repository.");
...
function isSourceFile(p: string) { return /\.(ts|tsx|js|jsx)$/.test(p) && !p.endsWith(".d.ts"); }
function isTestFile(p: string) { return /(\.test\.|\.spec\.)/.test(p); }
...
function extractExports(fileContent: string): string[] {
  ...
  while ((m = namedRe.exec(fileContent))) {
    m[1].split(",").map(s => s.trim().split(" as ")[0].trim()).forEach(n => { if (n) names.add(n); });
  }
  return [...names].sort();
}
...
function analyzeRepo(root: string) {
  ...
  const testFiles = fs.existsSync(testDir) ? listFilesRecursive(testDir, f => isSourceFile(f) && isTestFile(f)) : [];
  ...
  for (const f of testFiles) {
    ...
    const mentions = Array.from(new Set([...
      extractExports(content), ...extractDecorators(content)
    ])).sort();
...
// Tools
function buildAnalyzeRepositoryTool(): Tool<undefined, typeof analyzeRepoSchema> {
  return {
    ...
    execute: async (input) => {
      ...
      if (!fs.existsSync(repoRoot)) throw new Error(`Repository not found at ${repoRoot}`);
...
function deriveCapabilities(analysis: ReturnType<typeof analyzeRepo>): string[] {
  ...
  for (const k of Object.keys(analysis.api)) {
    ...
    for (const e of analysis.api[k].exports) if (/Decoration|decorate|Builder|Flavour/i.test(e)) cap.add("use-decoration-api");
  }
  if ([...allDecs].some(d => /override|extend/i.test(d))) cap.add("override-and-extend-decorations");
  if (Object.keys(analysis.tests).length > 0) cap.add("validate-with-tests");
  if (analysis.readme) cap.add("follow-readme-guides");
  return [...cap].sort();
}

function buildEnumerateCapabilitiesTool(): Tool<undefined, typeof enumerateCapabilitiesSchema> {
  return {
    ...
    execute: async (input) => {
      ...
      return { content: [ { type: "text", text: JSON.stringify({ capabilities, analysisSummary: {
...
function buildPlanFeatureTool(): Tool<undefined, typeof planFeatureSchema> {
  return {
    ...
    execute: async (input) => {
      const repoRoot = path.resolve(process.cwd(), input.repoPath);
      const steps: Array<{ step: number; action: string; tool?: string; arguments?: Record<string, any>; rationale: string }> = [];
      let i = 1;
      steps.push({ step: i++, action: "Analyze repository to enumerate APIs and decorators", tool: "analyze-repository", arguments: { repoPath: input.repoPath }, rationale: "Understand available building blocks." });
      steps.push({ step: i++, action: "List capabilities expected for developers", tool: "enumerate-capabilities", arguments: { repoPath: input.repoPath }, rationale: "Align the plan with supported capabilities." });
      // Suggest existing generic tools from mcp-module
      steps.push({ step: i++, action: "Select documentation prompt and gather relevant source file(s)", tool: "document-code", arguments: { filePath: "<target-file>" }, rationale: "Provide context and instructions for changes." });
      steps.push({ step: i++, action: "Apply code changes using unified diff patch", tool: "apply-code-change", arguments: { filePath: "<target-file>", patch: "<unified-diff>", dryRun: true }, rationale: "Validate changes safely before committing." });
      steps.push({ step: i++, action: "Commit code changes", tool: "apply-code-change", arguments: { filePath: "<target-file>", patch: "<unified-diff>", dryRun: false }, rationale: "Persist the update." });
      ...
      if (/decorat|flavour|override|extend|builder/i.test(input.feature)) {
        steps.unshift({ step: 0, action: "Use decorator tooling to insert/remove/modify decorators", tool: "decorator-tools", arguments: { action: "help" }, rationale: "Leverage specialized utilities for decoration patterns." });
        steps.forEach((s, idx) => s.step = idx + 1);
      }
      return { content: [ { type: "text", text: JSON.stringify({ plan: steps, notes: "Replace placeholder arguments like <target-file> and <unified-diff> based on the analysis output." }, null, 2) } ] };
    },
  };
}
...
function buildResourceTemplates(repoPath: string): DecorationResourceTemplate[] {
  ...
  return [
    {
      ...
      arguments: [ { name: "path", description: "Path under decoration/src to load, e.g. 'decoration/types.ts'", required: true } ],
...
export default function enrich(mcp: FastMCP) {
  ...
  for (const r of buildResourceTemplates(repoPath)) mcp.addResourceTemplate(r as any);
  return mcp;
}
